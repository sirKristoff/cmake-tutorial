...
#include <array>
#ifdef HAVE_CXX_STD_20_CONCEPTS
# include <concepts>
#endif
#include <cstdint>
#include <cuchar>
...
template< uint8_t nbits >  requires(32==nbits)
...
template<typename T>
concept BooleanType = std::convertible_to<T, bool>;

template<typename Params>
concept CrcAlgorithmParamsReq =
    NumericType<typename Params::value_t>    &&
    NumericType<decltype(Params::width)>     &&
    NumericType<decltype(Params::poly)>      &&
    NumericType<decltype(Params::rev_poly)>  &&
    NumericType<decltype(Params::init)>      &&
    NumericType<decltype(Params::xorout)>    &&
    BooleanType<decltype(Params::refin)>     &&
    BooleanType<decltype(Params::refout)>;

template< class Params >  requires CrcAlgorithmParamsReq<Params>
#else    // ifdef HAVE_CXX_STD_20_CONCEPTS
...
template< class Params >
#endif
const std::array<typename Params::value_t, (0b1U<<Params::width)>
algorithm<Params>::lookup_table;
